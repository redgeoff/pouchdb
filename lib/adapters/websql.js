...
var ATTACH_AND_DOCIDREV_STORE = quote('attach-rev-store');
...
var ATTACH_AND_DOCIDREV_STORE_DOCIDREV_INDEX_SQL =
  'CREATE INDEX IF NOT EXISTS \'attach-rev-docidrev-idx\' ON ' +
    ATTACH_AND_DOCIDREV_STORE + ' (doc_id, rev)';
var ATTACH_AND_DOCIDREV_STORE_ATTACH_INDEX_SQL =
  'CREATE INDEX IF NOT EXISTS \'attach-rev-digest-idx\' ON ' +
    ATTACH_AND_DOCIDREV_STORE + ' (digest)';
...
function WebSqlPouch(opts, callback) {
  ...
  // in this migration, we add the attach_and_docidrev table
  // for issue #2818
  function runMigration5(tx, callback) {

    function storeAttachmentMappings(tx) {
          ...
          tx.executeSql(sql, [], function (tx, res) {
            ...
            for (var i = 0; i < res.rows.length; i++) {
              ...
              for (j = 0; j < digests.length; j++) {
                ...
                mappings.push([doc._id, doc._rev, digest]);
              }
            }
            if (!mappings.length) {
              return nextPage();
            ...
            mappings.forEach(function (mapping) {
              var sql = 'INSERT INTO ' + ATTACH_AND_DOCIDREV_STORE +
                ' (doc_id, rev, digest)' +
                ' VALUES (?,?,?)';
              tx.executeSql(sql, mapping, function (tx, res) {
                if (++numDone === mappings.length) {
                  nextPage();
                }
              });
            });
    ...
    var attachAndRev = 'CREATE TABLE IF NOT EXISTS ' +
      ATTACH_AND_DOCIDREV_STORE +
      ' (doc_id, rev, digest)';
    tx.executeSql(attachAndRev, [], function (tx) {
      tx.executeSql(
          ATTACH_AND_DOCIDREV_STORE_DOCIDREV_INDEX_SQL, [], function (tx) {
        tx.executeSql(
          ATTACH_AND_DOCIDREV_STORE_ATTACH_INDEX_SQL, [],
          storeAttachmentMappings);
      });
    });
  }
  ...
  function onGetVersion(tx, dbVersion) {
    if (dbVersion === 0) {
      ...
      tx.executeSql(attachAndRev, [], function () {
        tx.executeSql(ATTACH_AND_DOCIDREV_STORE_DOCIDREV_INDEX_SQL);
        tx.executeSql(ATTACH_AND_DOCIDREV_STORE_ATTACH_INDEX_SQL);
      });
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    function writeDoc(docInfo, winningRev, deleted, callback, isUpdate,
      ...
      function finish() {
        ...
        // map docid/revs to attachment digests, which
        // we will need later during compaction
        function insertAttachmentMappings(callback) {
          var attsAdded = 0;
          ...
          function add(att) {
            var sql = 'INSERT INTO ' + ATTACH_AND_DOCIDREV_STORE +
              ' (doc_id, rev, digest) VALUES (?,?,?)';
            var sqlArgs = [id, rev, data._attachments[att].digest];
        ...
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          insertAttachmentMappings(function () {
  ...
  api._doCompaction = function (docId, rev_tree, revs, callback) {
    ...
    db.transaction(function (tx) {
      ...
      function deleteAttachmentsAndFinish() {
        if (!possiblyOrphanedDigests.length) {
          return;
        }
        possiblyOrphanedDigests.forEach(function (digest) {
          var sql = 'SELECT count(*) AS cnt FROM ' + ATTACH_AND_DOCIDREV_STORE +
            ' WHERE digest=?';
          tx.executeSql(sql, [digest], function (tx, res) {
            if (!res.rows.item(0).cnt) {
              // orphaned
              var sql = 'DELETE FROM ' + ATTACH_STORE + ' WHERE digest=?';
              tx.executeSql(sql, [digest]);
            }
          });
        });
      }
      ...
      tx.executeSql(sql, [docId], function (tx, result) {
        ...
        revs.forEach(function (rev) {
          ...
          tx.executeSql(sql, [docId, rev], function (tx) {
            var sql = 'SELECT digest FROM ' + ATTACH_AND_DOCIDREV_STORE +
              ' WHERE doc_id=? AND rev=?';
            var sqlArgs = [docId, rev];
            tx.executeSql(sql, sqlArgs, function (tx, res) {
              for (var i = 0; i < res.rows.length; i++) {
                possiblyOrphanedDigests.push(res.rows.item(i).digest);
              }
              var sql = 'DELETE FROM ' + ATTACH_AND_DOCIDREV_STORE +
                ' WHERE doc_id=? AND rev=?';
              tx.executeSql(sql, sqlArgs, function () {
                if (++numDone === revs.length) {
                  var sql = 'UPDATE ' + DOC_STORE + ' SET json=? WHERE id=?';
                  var sqlArgs = [vuvuzela.stringify(metadata), docId];
                  tx.executeSql(sql, sqlArgs, deleteAttachmentsAndFinish);
                }
              });
            });
          });
...
WebSqlPouch.destroy = utils.toPromise(function (name, opts, callback) {
  ...
  db.transaction(function (tx) {
    var stores = [DOC_STORE, BY_SEQ_STORE, ATTACH_STORE, META_STORE,
      LOCAL_STORE, ATTACH_AND_DOCIDREV_STORE];
